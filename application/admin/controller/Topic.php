...
Class Topic extends Controller
{
	...
	public function edit()
	{
		...
		if(empty($data['art_title']) || empty($data['art_content']))
		{
			...
			$result = DB::name('topic')->where('id',$id)->update(['art_title'=>$data['art_title'],'art_content'=>$data['art_content']]);
			if($result)
			{
				$this->success('编辑成功');
			}else{
	...
	public function search()
	{
		...
		$result = DB::name('topic')->where('article_title','like','%'.$data['search'].'%')->select();
