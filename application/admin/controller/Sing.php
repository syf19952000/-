...
Class Sing extends Controller
{
	public function index()
	{
		// 读取文章表，遍历显示
		$data = Db::name('sing')->select();
	...
	public function delete()
	{
		...
		// 进行删除操作
		$result = DB::name('sing')->where('id',$id)->delete();
		if($result)
		{
	...
	public function edit()
	{
		...
		if(empty($data['art_title']) || empty($data['art_content']))
		{
			...
			$result = DB::name('sing')->where('id',$id)->update(['art_title'=>$data['art_title'],'art_content'=>$data['art_content']]);
			if($result)
			{
				$this->success('编辑成功');
			}else{
	...
	public function search()
	{
		...
		$result = DB::name('sing')->where('article_title','like','%'.$data['search'].'%')->select();
