...
class Ju extends Commen
{
	...
	{	
		...
		$data = $_GET;
		$id = $data['id'];				// ju 表的id,查关联表的type_id
		$data2 = DB::name('ju')->where('id',$id)->find();
		$art_id = $data2['art_id'];
   ...
   { 
	...
	{
		$list = Db::name('ju')->order('id asc')->paginate(5);		
		$this->assign('data', $list);

		// $this->assign('data',$data);
		return $this->fetch('ju_list');	// view下article下的  名.html
	...
	public function editpage()
	{
		...
		// $datb['art_id']			= $art_id;
		$datb['art_zhu']		= $data['zhushi'];
		$res = DB::name('ju')->where('art_id',$data['art_id'])->update($datb);
		if($res)
		{
